...
class ZooKeeper extends EventEmitter {
    ...
    init(config) {
        ...
        if (this.config) {
            this.config = config ? _.defaults(config, this.config) : this.config;
        }
        if (this.logger) this.logger(`Calling init with ${util.inspect(config)}`);
        ...
        this.native.init.call(this.native, config);
    ...
    close(...args) {
        if (this.logger) this.logger(`Calling close with ${util.inspect(args)}`);
        return this.native.close(...args);
    }

    a_create(...args) {
        if (this.logger) this.logger(`Calling a_create with ${util.inspect(args)}`);
        return this.native.a_create(...args);
    }

    a_exists(...args) {
        if (this.logger) this.logger(`Calling a_exists with ${util.inspect(args)}`);
        return this.native.a_exists(...args);
    }

    aw_exists(...args) {
        if (this.logger) this.logger(`Calling aw_exists with ${util.inspect(args)}`);
        return this.native.aw_exists(...args);
    }

    a_get(path, watch, dataCb) {
        if (this.logger) this.logger(`Calling a_get with ${util.inspect({ path, watch, dataCb })}`);
        return this.native.a_get.call(this.native, path, watch, (rc, error, stat, data) => {
    ...
    aw_get(path, watchCb, dataCb) {
        if (this.logger) this.logger(`Calling aw_get with ${util.inspect({ path, watchCb, dataCb })}`);
        return this.native.aw_get.call(this.native, path, watchCb, (rc, error, stat, data) => {
            if (data && this.encoding) {
                return dataCb(rc, error, stat, data.toString(this.encoding));
            }
            return dataCb(rc, error, stat, data);
    ...
    a_get_children(...args) {
        if (this.logger) this.logger(`Calling a_get_children with ${util.inspect(args)}`);
        return this.native.a_get_children(...args);
    }

    aw_get_children(...args) {
        if (this.logger) this.logger(`Calling aw_get_children with ${util.inspect(args)}`);
        return this.native.aw_get_children(...args);
    }

    a_get_children2(...args) {
        if (this.logger) this.logger(`Calling a_get_children2 with ${util.inspect(args)}`);
        return this.native.a_get_children2(...args);
    }

    aw_get_children2(...args) {
        if (this.logger) this.logger(`Calling aw_get_children2 with ${util.inspect(args)}`);
        return this.native.aw_get_children2(...args);
    }

    a_set(...args) {
        if (this.logger) this.logger(`Calling a_set with ${util.inspect(args)}`);
        return this.native.a_set(...args);
    }

    // eslint-disable-next-line no-underscore-dangle
    a_delete_(...args) {
        if (this.logger) this.logger(`Calling a_delete_ with ${util.inspect(args)}`);
        // eslint-disable-next-line no-underscore-dangle
        return this.native.a_delete_(...args);
    }

    a_get_acl(...args) {
        if (this.logger) this.logger(`Calling a_get_acl with ${util.inspect(args)}`);
        return this.native.a_get_acl(...args);
    }

    a_set_acl(...args) {
        if (this.logger) this.logger(`Calling a_set_acl with ${util.inspect(args)}`);
        return this.native.a_set_acl(...args);
    }

    add_auth(...args) {
        if (this.logger) this.logger(`Calling add_auth with ${util.inspect(args)}`);
        return this.native.add_auth(...args);
    }

    mkdirp(p, cb) {
        if (this.logger) this.logger(`Calling mkdirp with ${util.inspect({ p, cb })}`);
        return mkdirp(this, p, cb);
    }

    a_sync(...args) {
        if (this.logger) this.logger(`Calling a_sync ${util.inspect(args)}`);
